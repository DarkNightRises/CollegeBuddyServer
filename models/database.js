...
query = client.query('create table if not exists Answer(ID serial primary key,test_id int,student_id int,choice_id int,FOREIGN KEY (choice_id) REFERENCES Choices(ID) on delete cascade on update cascade,FOREIGN KEY (test_id) REFERENCES Test(ID) on delete cascade on update cascade,FOREIGN KEY (student_id) REFERENCES Student(ID) on delete cascade on update cascade)');
 ...
 query = client.query('create table if not exists Review(ID serial PRIMARY KEY,student_id int, review_text text NOT NULL, teacher_id int,subject_id int,FOREIGN KEY (student_id) REFERENCES Student(ID) on delete cascade on update cascade,FOREIGN KEY (teacher_id) REFERENCES Teacher(ID) on delete cascade on update cascade,FOREIGN KEY (subject_id) REFERENCES Subject(ID) on delete cascade on update cascade)');
