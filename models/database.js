...
var query = client.query('CREATE TABLE IF NOT EXISTS items(id SERIAL PRIMARY KEY, text VARCHAR(40) not null, complete BOOLEAN)');
 ...
 query = client.query('create table if not exists Student(ID serial primary key, Name Text NOT NULL,section_Id int Not Null,College_Id int not null,Email Text NOT NULL,Mob_No Text NOT NULL,Password Text NOT NULL,gcm_id text,device_id text,FOREIGN KEY (College_Id) REFERENCES College(ID) on delete cascade on update cascade,FOREIGN KEY (section_Id) REFERENCES Section(ID) on delete cascade on update cascade)');
